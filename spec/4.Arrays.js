	...
	describe("can mutate", () => {
		...
		describe("can behave as a stack", () => {
			...
			describe("from head", () => {
				...
				it("shift removes the first element", () => {
					array.shift();
					expect([2,3]).toEqual(array);
				});

				it("shift returns the value of the first element", () => {
	...
	describe("non mutating operations", () => {
		...
		describe("iterators", () => {
			...
			it("filter selects elements from the array", () => {
				expect([1,3]).toEqual(array.filter(n => n % 2 === 1));	// filtra els parells
			});

			it("map applies a function to each element", () => {
				expect([2,4,6]).toEqual(array.map(n => n * 2));	// multiplica cada valor
			});

			it("reduce(fn, initial) applies an accumulator operation", () => {
				expect(6).toEqual(array.reduce((s, n) => s + n, 0));	// agrega SUM tots els valors
			});

			it("every evaluates if a condition is satisfied by all elements", () => {
				expect(true).toBe(array.every(n => n >= 0)); 		// tots els valors son més grans de 0
				expect(false).toBe(array.every(n => n % 2 === 0)); 	// tots els valors son parells?
				expect(false).toBe(array.every(n => n === -1)); 	// tots els valors son -1
			});
